...
export function PlanApproval() {
  ...
  const handleApprove = async () => {
    try {
      ...
      setSuccessMessage('Plans approved successfully')
      setTimeout(() => navigate('/checker'), 2000)
    } catch (err) {
      setError('Failed to approve plans')
      console.error('Error:', err)
    }
  }
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setSuccessMessage('Plans rejected successfully')
      setTimeout(() => navigate('/checker'), 2000)
    } catch (err) {
      setError('Failed to reject plans')
      console.error('Error:', err)
    }
  }
            ...
            {plans.length === 0 ? (
              ...
              plans.map((plan) => (
                <TableRow key={plan.id}>
                  ...
                  <TableCell>{plan.editedBy}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
